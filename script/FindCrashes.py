	...
	def detect(self):
		...
		for fn in os.listdir(tcFolder):
			...
			if os.path.isdir(tcFolder_fn):
				...
				print fn, len(allLogDict)," bugs by ",str(allBugNum) ," EA+IA tcs"
				fw.write(fn + "\t"+str(len(allLogDict))+"\tbugs by "+str(allBugNum) +"\tEA+IA tcs"+"\n")
				
				print fn, len(eaLogDict)," bugs by ",str(eaBugNum) ," EA tcs"
