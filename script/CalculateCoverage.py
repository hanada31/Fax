	...
	def writeHtml(self):
		...
		for pkg in self.coverDict.keys():
			...
			for cls in self.coverDict[pkg].keys():
				...
				fw.write('<h3>  <font color="#336699"> class name: ' + cls+'</font></h3>')
		...
		while 1:
			...
			for line in lines:
				...
				if pkg =='':
					continue
				method = line[1:][:-1]
				self.methodDict[method]=0
				self.coverDict[pkg][classA][method]=False
		file.close()
	...
	def calMethodCoverage(self):
		...
		while 1:
			...
			for line in lines:
				line = line.strip('\r\n')
				method = line[1:][:-1]
				if  self.methodDict.has_key(method):
					if self.methodDict[method] == 0:
						self.methodDict[method] = 1
						self.methodCovNum+=1
					else:
						self.methodDict[method] += 1
				classA = self.getClass(line)
				...
				if pkg =='':
					continue
				if pkg in self.coverDict and classA in self.coverDict[pkg] and method in self.coverDict[pkg][classA]:
					if self.coverDict[pkg][classA][method]==False:
						self.coverDict[pkg][classA][method]=True
		file.close()	
